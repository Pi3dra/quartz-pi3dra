// copied from https://github.com/MikeKneeB/quartz-site/blob/main/extra/components/scripts/overlayexplorer.inline.ts
...
function setFolder(folderPath: string, open: boolean) {
  ...
  const collapseIcon = folderEntry.getElementsByTagName(
      "svg"
  )[0] as unknown as MaybeHTMLElement
...
function setupOverlayExplorer() {
  ...
  function toggleFolder(evt: MouseEvent) {
    ...
    const folderPath = (target.parentNode as HTMLElement)?.getAttribute("data-ol-selector-for")
    const childrenList = document.querySelector(
      `[data-ol-children-for='${folderPath}']`
    ) as MaybeHTMLElement
    if (!childrenList) return

    const collapseIcon = (target.parentNode as HTMLElement)?.getElementsByTagName(
        "svg"
    )[0] as unknown as MaybeHTMLElement
    if (!collapseIcon) return
